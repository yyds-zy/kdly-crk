 ...
 * 原材料
 */
public class InOneActivity extends Activity implements Server_Response.OnGetSeverLister, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_in_one);
...
//    };
    ...
    private void zifuchaunchuli_huowei(){
        ...
        TLog.e("扫描结果产品zifuchaunchuli_huowei===",aa);
            if(!spec_number.equals(temp)){
                Toast.makeText(InOneActivity.this,"库房"+spec_number+"下没有此库位号", Toast.LENGTH_SHORT).show();
                et_jioadian.setText("");
            } else
            {
                request_pipei(aa);
    ...
    private void zifuchaunchuli_cinv(){
                ...
                if(str.length==9){
                    ...
                    cinv_clear();
                    et_jioadian.setText("");
                    Toast.makeText(InOneActivity.this, "二维码格式不符", Toast.LENGTH_SHORT).show();
                }
    ...
    private void request_Rk(){//点击入库
        Call<ResponseBody> rk = ServerUtil.getInstance().getServer().rk("0", qrCode, tv_base_qtyn.getText().toString(), positionCode , "");
    ...
    public void onSuccess(Object response) {
        if (response instanceof  PiPeiResult){//判断此货位码是否可用，返回结果
            ...
            if("ky".equals(sfky)){
                ...
                Toast.makeText(InOneActivity.this, ""+sfky, Toast.LENGTH_SHORT).show();
                kuwei_clear();
            }
            et_jioadian.setText("");

        } else if (response instanceof Ruku_Result){//执行入库操作 返回结果
            Ruku_Result ruku_result = (Ruku_Result) response;
            if (ruku_result.getIsIn().equals("true")){
                Toast.makeText(InOneActivity.this,""+ruku_result.getIsInmsg(),Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(InOneActivity.this,"入库成功",Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    public void onClick(View v) {
        if (v== img_fanhui){//关闭当前页
            ...
            if("".equals(cinvStd)) {
                Toast.makeText(InOneActivity.this,"產品信息不能為空！",Toast.LENGTH_SHORT).show();
            }
            else
            if("".equals(positionCode)){
                Toast.makeText(InOneActivity.this,"貨位信息不能為空！",Toast.LENGTH_SHORT).show();
            ...
            AlterDialog dialog = new AlterDialog(InOneActivity.this, num, new OnOkClickListener() {
                @Override
                    ...
                    if (null != aa && aa.length() > 0 ){
                        ...
                        if (aa.length() == 10) {//货位
                            ...
                            if (isCan){
                                ...
                                Utils.showDialogVersion2(context, "结果返回中，请稍后重试");
                            }
                        } else if (aa.split("~").length == 9) {//产品
                            TLog.e("扫描结果产品===",aa);
                            zifuchaunchuli_cinv();
